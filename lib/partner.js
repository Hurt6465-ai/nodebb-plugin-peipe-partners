...
function localizeOptions(baseOptions, dict) {
  const cloneList = list => (list || []).map(item => Object.assign({}, item, {
    label: dict[item.key] || item.label || item.value || item.key || ''
  }));
  return {
    countries: cloneList(baseOptions.countries),
    languages: cloneList(baseOptions.languages),
    genders: cloneList(baseOptions.genders)
  };
}
...
async function getOptions(req) {
  ...
  return { ok: true, locale, options: localizeOptions(options, dict) };
}
