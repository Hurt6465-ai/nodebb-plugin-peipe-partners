...
async function getCandidateUids() {
  let uids = [];
  try {
    uids = await db.getSortedSetRevRange('users:joindate', 0, CONFIG.maxPoolScan - 1);
  } catch (e) {
    uids = [];
  }
  return uids.map(uid => Number(uid)).filter(uid => uid > 0);
}
  ...
  cache.buildingPool = (async () => {
    ...
    for (let i = 0; i < uids.length; i += CONFIG.batchSize) {
      ...
      for (const item of users) {
        ...
        if (decorated) pool.push(decorated);
      }
    }
    cache.pool = pool;
...
async function list(req) {
  ...
  if (mode === 'nearby') {
    ...
    candidates = candidates.map(item => ({
      item,
      score: (onlineSet.has(Number(item.uid)) ? 100 : 0) + languageScore(viewer, item) + (item.picture ? 3 : 0) + (item.bio ? 2 : 0) + Math.min(Number(item.lastonline || 0) / 10000000000000, 0.5) + (hashString(`${seed}:${item.uid}`) % 1000) / 1000
...
async function saveProfile(uid, body) {
  ...
  const native = cleanText(body.language_fluent || body.native || body.native_language);
  const learning = cleanText(body.language_learning || body.learning || body.target_language);
  ...
  if (!countryValues.has(country) || !languageValues.has(native) || !languageValues.has(learning) || !genderValues.has(gender) || !age || age < 13 || age > 99) {
    return { ok: false, error: 'invalid-profile' };
  }

  await user.setUserFields(uid, {
    language_flag: country,
    language_fluent: JSON.stringify([native]),
    language_learning: JSON.stringify([learning]),
    gender,
    age
  });
  cache.poolBuiltAt = 0;
...
module.exports = {
  ...
  markChatted,
  greet,
  _private: { CONFIG, cache, parseMulti, toLangCodes, normalizeGender, matchCountryCode, distanceBucket }
};
