...
async function getOnlineSet() {
  // 如果缓存未过期，直接返回
  if (cache.onlineBuiltAt && now() - cache.onlineBuiltAt <= CONFIG.onlineTtlMs) {
  ...
  cache.buildingOnline = (async () => {
    let values = [];
    try {
      // 只取最近 onlineActiveMs 内有活动的 uid
      values = await db.getSortedSetRevRangeByScore(
        'users:online',
        0,
        -1,
        now() + 60 * 1000,          // 上限稍微宽一点
        now() - CONFIG.onlineActiveMs
      );
    } catch (e) {
      values = [];
    }
...
function applyOnline(userData, onlineSet) {
  const isOnline = onlineSet.has(Number(userData.uid));
